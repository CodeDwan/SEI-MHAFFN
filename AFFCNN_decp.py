...
class multi_CNN(nn.Module):
    def __init__(self, class_mum):
        ...
        self.conv2d_b2 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=64, kernel_size=5, padding='same'),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(1, 4)),
        )
        ...
        self.conv2d_c1 = nn.Sequential(
            nn.Conv2d(in_channels=2, out_channels=16, kernel_size=7, padding='same'),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(1, 4)),
        )
...
if __name__ == "__main__":
    ...
    model_path = './Modelsave/singlesourceFFN_decp.pth'  # 模型保存path
    pic_path = './Modelsave/figure/singlesourceFFN_decp.jpg' # 图片保存路径
    tsne_path = './Modelsave/figure/singlesourceFFN_decp_tsne.jpg'
    bsize = 32     # batch_size
    ...
    # package the dataset
    train_ds = TensorDataset(torch.Tensor(x_train_b), torch.Tensor(x_train_b), torch.Tensor(x_train_b),
                             torch.Tensor(y_train))
    train_dl = DataLoader(train_ds, batch_size=bsize, shuffle=True)
    val_ds = TensorDataset(torch.Tensor(x_val_b), torch.Tensor(x_val_b), torch.Tensor(x_val_b),
                           torch.Tensor(y_val))
    val_dl = DataLoader(val_ds, batch_size=bsize, shuffle=True)
    test_ds = TensorDataset(torch.Tensor(x_test_b), torch.Tensor(x_test_b), torch.Tensor(x_test_b),
                            torch.Tensor(y_test))
    ...
    def FLOPs():
        tensor_a = torch.rand(x_test_b.shape)[:2]
        tensor_b = torch.rand(x_test_b.shape)[:2]
        tensor_c = torch.rand(x_test_b.shape)[:2]
