...
class multi_CNN(nn.Module):
    def __init__(self, class_mum):
        ...
        # rx_equfre_data_b的卷积结果
        self.fc_b = nn.Sequential(
            nn.LazyLinear(256*self.linearchannel),
            nn.ReLU(),
            nn.Dropout(p=0.3),
        )
        # rx_equfre_data_c的卷积结果
...
if __name__ == "__main__":
    ...
    # package the dataset
    train_ds = TensorDataset(torch.Tensor(x_train_c), torch.Tensor(x_train_c), torch.Tensor(x_train_c),
                             torch.Tensor(y_train))
    train_dl = DataLoader(train_ds, batch_size=bsize, shuffle=True)
    val_ds = TensorDataset(torch.Tensor(x_val_c), torch.Tensor(x_val_c), torch.Tensor(x_val_c),
                           torch.Tensor(y_val))
    val_dl = DataLoader(val_ds, batch_size=bsize, shuffle=True)
    test_ds = TensorDataset(torch.Tensor(x_test_c), torch.Tensor(x_test_c), torch.Tensor(x_test_c),
                            torch.Tensor(y_test))
   ...
   # 训练过程
    ...
    def FLOPs():
        tensor_a = torch.rand(x_test_c.shape)[:2]
        tensor_b = torch.rand(x_test_c.shape)[:2]
        tensor_c = torch.rand(x_test_c.shape)[:2]
