...
class multi_CNN(nn.Module):
    def __init__(self, class_mum):
        ...
        self.conv2d_b1 = nn.Sequential(
            nn.Conv2d(in_channels=2, out_channels=16, kernel_size=5, padding=2),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(1, 4)),
        )
        self.conv2d_b2 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=64, kernel_size=7, padding=3),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(1, 4)),
        )
...
if __name__ == "__main__":
    ...
    model_path = './Modelsave/MHAFFN_SNR0.pth'  # 模型保存path
    pic_path = './Modelsave/figure/MHAFFN_SNR0.jpg' # 图片保存路径
    tsne_path = './Modelsave/figure/MHAFFN_tsne_SNR0.jpg'
    ...
    x_train_a = addwgn(x_train_a,snr=SNR)
    x_val_a = addwgn(x_val_a, snr=SNR)
    x_test_a = addwgn(x_test_a, snr=SNR)
    ...
    x_train_b = addwgn(x_train_b,snr=SNR)
    x_val_b = addwgn(x_val_b, snr=SNR)
    x_test_b = addwgn(x_test_b, snr=SNR)
    ...
    x_train_c = addwgn(x_train_c,snr=SNR)
    x_val_c = addwgn(x_val_c, snr=SNR)
    x_test_c = addwgn(x_test_c, snr=SNR)
